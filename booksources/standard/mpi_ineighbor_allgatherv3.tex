...
\hbox to 20pt{MPI_Ineighbor_allgatherv (\hss} \\
 ...
 & recvcounts & const int[] & INTEGER(*)  & IN \\&\hbox to 0pt{\footnotesize\sl non-negative integer array (of length indegree) containing the number of elements that are received from each neighbor\hss} \\ [+3pt] 
 & displs & const int[] & INTEGER(*)  & IN \\&\hbox to 0pt{\footnotesize\sl integer array (of length indegree). Entry \mpicode{i} specifies the displacement (relative to \mpiarg{recvbuf}) at which to place the incoming data from neighbor \mpicode{i}\hss} \\ [+3pt] 

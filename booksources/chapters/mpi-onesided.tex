...
\item You can let MPI do the allocation, so that MPI can perform various
  ...
  \item If a communicator is on a shared memory (see
    ...
    \emph{MPI shared memory}\index{memory!shared, MPI};
...
The Put/Get/Accumulate routines have an \indexmpishow{MPI_Op}
...
\indexmpishow{MPI_Win_start}\indexmpishow{MPI_Win_complete}:
\begin{lstlisting}
...
to access a target.
You can either acquire a lock on a specific rank with \indexmpishow{MPI_Win_lock},
or on all ranks (of a communicator) with \indexmpishow{MPI_Win_lock_all}.
...
respectively \indexmpixref{MPI_Win_unlock_all}{MPI_Win_unlock}.

\begin{exercise}
  \label{ex:lockandfetch}
  \exercisesource{lockfetch}
...
\begin{exercise}
  ...
  \exercisesource{lockfetchshared}
...
\indexmpixref{MPI_Win_unlock_all}{MPI_Win_unlock}.

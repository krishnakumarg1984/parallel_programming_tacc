...
\begin{mpifour}
  The routines \indexmpishow{MPI_Info_get} and \indexmpishow{MPI_Info_get_valuelen}
...
\item Most commonly, an error handler is associated with a communicator:
  ...
  (and likewise it can be retrieved with \indexmpishow{MPI_Comm_get_errhandler});
\item other possibilities are
  \indexmpishow{MPI_File_set_errhandler},
...
\indexmpishow{MPI_Errhandler_create},
...
\index{communication!non-local|(textbf}
...
is \emph{non-local}: its completion may be depend on
whether the corresponding receive has been posted.
A \indextermbusdef{local}{operation} is one that is not non-local.

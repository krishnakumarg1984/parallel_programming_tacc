...
\begin{mplnote}{Requests from non-blocking calls}
  The request object, which is in the class \indexmpldef{irequest},
  is a function result, not a parameter pass by reference.
  The single wait call is a method of the \indexmplshow{irequest}
  class.
  %
...
To deal with multiple requests, there is a \indexmpldef{irequest_pool}
that requests can be pushed onto:
...
\begin{fortrannote}
  ...
  \fverbatimsnippet[examples/mpi/f/irecv_source.F90]{waitforany-f}
...
\begin{mplnote}{Wait any}
  ...
  meaning that you can \lstinline+push_back+ onto it.
...
\Level 3 {Polling with MPI Wait any}
...
\Level 3 {Wait for some requests}
...
This is not automatic. See section~\ref{sec:progress} for more discussion.

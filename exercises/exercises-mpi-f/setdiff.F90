...
#define N 10
  ...
  integer :: nprocs, procno,sendto,recvfrom,proc,i,ierr
...
contains
  ...
  subroutine print_distarray(local,comm,sort)
    ...
    integer :: n,nprocs,procno, top,s,tmp,ierr

    n = size(local)
    call MPI_Comm_size(comm,nprocs,ierr)
    call MPI_Comm_rank(comm,procno,ierr)
    if (procno==0) Allocate( global(n*nprocs) )
    call MPI_Gather( local,n,MPI_INTEGER,global,n,MPI_INTEGER,0,comm,ierr)
    if (procno==0) then
       if (sort) then
          do top=nprocs*n,1,-1
             do s=1,top-1
